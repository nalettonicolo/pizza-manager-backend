...
export default function Pony() {
  ...
  return (
    <div style={{ padding: 24 }}>
      <h1 className="dashboard-page-title">Pony{operatoreLabel ? ` — ${operatoreLabel}` : ""}</h1>
      <p style={{ color: "#666", marginBottom: 16 }}>Consegne asporto{operatoreLabel ? ` (${operatoreLabel})` : ""}</p>

      {loading && orders.length === 0 ? (
        <p style={{ color: "#888" }}>Caricamento...</p>
      ) : orders.length === 0 ? (
        <p style={{ color: "#888" }}>Nessuna comanda da ritirare per consegna.</p>
      ) : (
