...
const CATEGORIE = [
  ...
  { value: "utenze", label: "Utenze (luce, gas, acqua)" },
  { value: "manutenzione", label: "Manutenzione locale" },
  { value: "altro", label: "Altro overhead" },
];

export default function SpeseLocalePage() {
  const { data, setData, ready } = useTenantLocalJson("contabilita_spese_locale", { spese: [] });
  const [dataSpesa, setDataSpesa] = useState(() => new Date().toISOString().slice(0, 10));
  const [categoria, setCategoria] = useState("affitto");
  ...
  return (
    <AdminModuleShell
      title="Spese gestione locale"
      lead="Costi fissi e variabili del locale: affitto, bollette, manutenzione e altre voci di struttura."
      specTitle="Esempi di categorie"
      specChildren={
        <p style={{ margin: 0 }}>Affitto, utenze, pulizie, manutenzione impianti, assicurazioni locali, ecc.</p>
      }
    >
      <div
        style={{
      ...
      {data.spese.length === 0 ? (
        <p style={{ padding: 16, color: "#94a3b8", fontSize: 14 }}>Nessuna spesa locale.</p>
      ) : null}
    </AdminModuleShell>
  );
}
