...
-- Modifiche puntuali correnti (file rotativo): sql/sql_upgrade.sql e moduli sql/modules/ (es. 11 rider)
...
DECLARE r RECORD;
BEGIN
  FOR r IN (SELECT unnest(ARRAY['prodotti_menu_pubblico','prodotto_ingrediente','ProdottoIngrediente','ingredienti','configurazione_costi','ConfigurazioneCosti','Prodotto','cottura','formati','Categoria','categorie','Allergene','allergeni','impasti','punti_vendita','tenants','Tenant','Ingrediente','Ordine','RigaOrdine','riga_ordine']) AS n)
  LOOP
...
DROP TABLE IF EXISTS public.notifiche_outbox CASCADE;
DROP TABLE IF EXISTS core.consegna_percorso_ordine CASCADE;
DROP TABLE IF EXISTS core.ordine_consegna_evento CASCADE;
DROP TABLE IF EXISTS core.rider_posizione CASCADE;
DROP TABLE IF EXISTS core.consegna_percorso CASCADE;
DROP TABLE IF EXISTS core.turno_rider CASCADE;
DROP TABLE IF EXISTS core.rider CASCADE;
...
CREATE TABLE core.prodotti (
    ...
    attivo BOOLEAN DEFAULT true,
    visibile_online BOOLEAN DEFAULT true,
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now(),
    deleted_at TIMESTAMPTZ
);
...
CREATE VIEW public.prodotti_menu_pubblico AS
  SELECT id, nome, descrizione, prezzo, attivo, ordine, immagine_url, visibile_online,
         ...
         created_at AS "createdAt", updated_at AS "updatedAt", deleted_at AS "deletedAt"
  FROM core.prodotti
  WHERE deleted_at IS NULL AND (attivo = true OR attivo IS NULL) AND (visibile_online = true OR visibile_online IS NULL);
...
-- ============================================================
-- FASE 6 — RIDER / CONSEGNE ENTERPRISE (sql/modules/11)
-- ============================================================
...
-- Permessi anon (menu e tenant pubblico senza login)
