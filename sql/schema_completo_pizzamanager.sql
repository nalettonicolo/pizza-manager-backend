...
CREATE OR REPLACE FUNCTION "public"."set_numero_ordine"() RETURNS "trigger"
    ...
    AS $$
BEGIN
    -- Se numero_ordine � gi� valorizzato non lo tocca
...
-- 1) Colonna revision (idempotente su DB gi� migrati senza colonna)
...
--   (o DB gi� allineato a quello snapshot).
--
...
--         oppure su DB gi� allineato. Sicuro da rieseguire (IF NOT EXISTS / blocchi DO).
--
...
-- Nota: con backend Node/Nest che fa le query, spesso RLS non � usato e l?isolamento
-- � garantito dal middleware che inietta sempre tenantId nelle query.
...
-- ============================================================
-- AGGIUNGI deleted_at SE MANCA (DB gi� esistenti / vecchie run)
-- Cos� gli indici parziali WHERE deleted_at IS NULL funzionano.
-- ============================================================
...
-- Super Admin usa `public.tenants`, che nel dump remoto � una VISTA su `admin.tenants`.
...
BEGIN
  ...
  -- Stesso slug su pi� righe: suffisso deterministico da id (senza alterare la prima riga del gruppo).
  UPDATE admin.tenants t
  ...
  -- CREATE OR REPLACE VIEW non pu� riallineare colonne se la vista esistente ha ordine/nomi diversi
  -- (es. slug gi� in posizione 3 in produzione vs dump con solo piano) ? 42P16.
  -- Ricreazione completa: DROP + CREATE. CASCADE solo se dipendenze da altre viste (raro su tenants).
  IF relkind = 'r' THEN
    RAISE NOTICE 'public.tenants � una tabella (relkind=r): non ricreare vista; verifica PostgREST.';
...
BEGIN
  ...
  IF relkind = 'r' THEN
    RAISE NOTICE 'public.tenants � una tabella: salta ricreazione vista.';
    RETURN;
  END IF;
...
-- La vista public.tenants con CASE su `piano` (? core.piano_saas) NON � aggiornabile automaticamente:
...
BEGIN
  ...
  IF relkind = 'r' THEN
    RAISE NOTICE 'public.tenants � una tabella: salta.';
    RETURN;
  END IF;
...
-- Helper: sottoinsieme parametri_operativi sicuro per anon (mod. 40)
...
-- Categorie catalogo admin (core.categorie) per vetrina: allinea tab al nome/ordine reali anche se categoria_nome nel menu � assente
...
BEGIN
  ...
  COMMENT ON COLUMN admin.tenants.sito_web_cliente IS 'URL completo del sito web del cliente (marketing, Google Sites, ecc.); non � usato per la risoluzione tenant';
END $$;
...
--
-- Uso: database gi� inizializzato (es. dopo schema bootstrap). Esegui in
...
-- -----------------------------------------------------------------------------
-- 1) core.prodotti ? visibilit� menu online
-- -----------------------------------------------------------------------------
...
-- sql/schema_completo_pizzamanager.sql ? non duplicarle qui se il DB � gi� allineato.
...
-- Non � la password reale in auth.users: � un archivio opzionale che il titolare aggiorna quando crea/resetta l?accesso.
...
-- Il Super Admin � identificato da public.utenti_ruoli (ruolo = 'superadmin', attivo).

-- -----------------------------------------------------------------------------
-- 1) Vista ruoli_pizzeria: include tutte le righe se l'utente corrente � superadmin
-- -----------------------------------------------------------------------------
DROP VIEW IF EXISTS public.ruoli_pizzeria CASCADE;
...
--
-- App (senza DDL qui): Admin Magazzino/Contabilit� usa ancora localStorage per tenant;
...
--   pi� ritiro_ogni_min, pizze_ogni_15_min, consegne_ogni_min, ?
--
...
COMMENT ON TABLE public.staff_password_note IS
  'Nota password accesso staff (archivio titolare). RLS: tenant_admins del tenant o superadmin (utenti_ruoli). Non � la password Auth.';
...
BEGIN
  IF to_regclass('public.subscriptions') IS NOT NULL THEN
    ...
    COMMENT ON COLUMN public.subscriptions.sconto_annuale_percent IS
      'Sconto % sul totale 12 mensilit� se ciclo annuale; NULL se mensile.';
  END IF;
  ...
  THEN
    ...
    IF NOT v_is_staff_cassa THEN
      ...
      IF v_inside IS DISTINCT FROM true THEN
        RAISE EXCEPTION 'L''indirizzo di consegna � fuori dall''area coperta dal locale.';
      END IF;
    END IF;
  END IF;
...
-- Base tabella turni cassa (se il dump remote_schema non � mai stato applicato).
...
-- FK verso punti_vendita solo se � tabella base (relkind 'r'): su alcuni DB � una VIEW ? niente FK.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Audit operazioni cassa (append-only) per tracciabilit� enterprise.
...
COMMENT ON FUNCTION public.stripe_refund_allowed(UUID, UUID) IS
  'Solo Edge (service_role): verifica se l''utente pu� rimborsare l''ordine.';
...
COMMENT ON FUNCTION public.create_order_with_items(
  ...
  'Crea ordine + righe. web_cliente_user_id valorizzato se auth.uid() � cliente del tenant.';
...
-- ordini non riordinabili al ricalcolo percorso (es. gi� in forno).
--
...
COMMENT ON TYPE core.stato_delivery IS
  'Ciclo consegna rider (affianca core.ordini.stato_consegna TEXT per compatibilit�).';
...
BEGIN
  ...
  COMMENT ON COLUMN core.ordini.assegnato_rider_at IS 'Quando l''ordine � stato assegnato al rider.';
...
COMMENT ON TABLE public.magazzino_movimenti IS
  'Movimenti di carico/scarico; prodotto_id opzionale se il movimento � aggregato o non legato al listino.';
...
COMMENT ON FUNCTION public.pm_core_tenant_access(uuid) IS
  'RLS core: true se auth.uid() � superadmin, staff/cliente del tenant, o rider (core.rider.auth_user_id).';
...
COMMENT ON FUNCTION public.create_order_with_items(
  ...
  'Crea ordine + righe. Hardening tenant + cliente web; web_cliente_user_id se auth � cliente del tenant; telefono_ritiro opzionale.';
...
COMMENT ON COLUMN public.staff_archivio_dipendenti.scheda_disabilitata IS
  'Se true la scheda HR � considerata non attiva (es. archivio storico) senza eliminare i dati.';
...
COMMENT ON TABLE public.magazzino_fornitori IS 'Fornitori magazzino con listino righe in JSON (descrizione, prezzo, unit�).';
COMMENT ON TABLE public.magazzino_ddt IS 'DDT in entrata; riferimento per contabilit� fatture.';
...
-- ---------- BEGIN CONSOLIDAMENTO 2026-05-30-b (idempotency + contabilit� estesa) ----------
...
-- ---------- BEGIN CONSOLIDAMENTO 2026-05-30-c (Stripe online confirm + webhook per tenant) ----------
