...
- **2026-08-30 — batch fix audit OWASP (moduli SQL 137–144)**: IDOR critico chiuso su `core.ordini` (un cliente poteva PATCH/DELETE gli ordini di altri via `/rest/v1/Ordine`, la policy RLS unica "ALL" non distingueva SELECT da scrittura); accesso non autorizzato a `attrezzature_catalogo`/`staff_password_note`; bypass hardcoded per un'email specifica rimosso da `delivery_update_stato_consegna`/`delivery_mark_consegnato`; `punti_vendita` e `clienti` non filtravano più lo staff disattivato; IDOR su `pm_agente_quota_*` (bypass `service_role` aggiunto per non rompere la chiamata interna di `agente-chat`); XSS nella mappa consegne Cassa (icona Leaflet costruita per concatenazione di stringa) e negli stessi 4 punti JSON-LD dell'entry precedente. Rimosso in giornata anche un meccanismo "pony a slot" (login condiviso tra due rider) introdotto per errore su una premessa sbagliata — ogni pony ha il proprio login individuale.
- **2026-09-04/21 — app desktop Windows (Electron)**: wrapper nativo che carica sempre il sito live (`pizzamanager.it/login`) in una `BrowserWindow` — nessuna build offline separata, ogni deploy web raggiunge automaticamente anche il desktop. Riconoscimento tenant via Partita IVA nel wizard di installazione (pagina custom NSIS, obbligatoria: senza una P.IVA riconosciuta l'installazione non prosegue), verificata da una nuova Edge Function pubblica `verifica-piva-tenant` (checksum ufficiale + match esatto su tenant attivo, mai una ricerca per nome); mostra anche la sede legale se diversa da quella operativa (modulo SQL 145, `admin.tenants.sede_legale`) e salva il collegamento in locale così login/header mostrano da subito nome e logo della pizzeria riconosciuta invece del marchio generico. Aggiornamento automatico via `electron-updater`, applicato in silenzio (`quitAndInstall(true, true)`) senza riaprire il wizard di installazione su un'app già installata; canale spostato da un "ponte" sul dominio a **GitHub Releases** (repo pubblica), per scollegare la pubblicazione di un installer dal deploy completo del sito — con una versione-ponte (1.0.5) pubblicata su entrambi i canali per non lasciare indietro chi aveva già installato una versione precedente. Corretti in corso d'opera: bug per cui i dati salvati da Superadmin → Clienti (es. `prova_valida_fino`) non comparivano mai riaprendo il modale — `getTenants()` tentava per prima una selezione di colonne "povera" che riusciva sempre banalmente, non arrivava mai a provare quella completa (test di regressione aggiunto); pulsante "▶ Stress test" di Cassa senza `onClick`; manifest PWA distinto per contesto staff (`/login`) vs cliente finale (`/negozio`, invariato).
