...
export class OperativeService {
  ...
  async listOrdini(
    ...
    const stato =
      opts.stato && String(opts.stato).trim()
        ? String(opts.stato).trim()
        : null;
  ...
  async batchProdottoIngredientiMerged(
    ...
    try {
      ...
      const ingRows = await this.prisma.$queryRaw<
        Record<string, unknown>[]
      >(Prisma.sql`
        SELECT *
        FROM core.ingredienti
      ...
      for (const pid of uniqueIds) {
        ...
        const ids = prRows.map((row) => row.ingrediente_id).filter(Boolean);
        let ordered = ids.map((id) => byIng.get(id)).filter(Boolean);
