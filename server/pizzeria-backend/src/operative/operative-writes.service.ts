...
export class OperativeWritesService {
  ...
  private mapRigaCliente(r: Record<string, unknown>) {
    ...
    return {
      ...
      ingredienti_cottura_summary:
        m.ingredienti_cottura_summary ?? m.ingredientiCotturaSummary,
    };
  }
  ...
  async turnoApri(
    ...
    if (!pvRows?.length) {
      throw new ForbiddenException(
        'Punto vendita non valido per questo tenant',
      );
    }
  ...
  async createOrder(
    ...
    const statoRaw =
      (body.stato && String(body.stato).trim()) || 'IN_PREPARAZIONE';
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const turnoSql =
        body.turno_operatori_id != null &&
        Number.isFinite(Number(body.turno_operatori_id))
      ...
      try {
        ...
        this.logger.warn(
          `createOrder insert esteso fallito, uso minimale: ${String(e)}`,
        );
        const inserted = await tx.$queryRaw<{ id: string }[]>(Prisma.sql`
  ...
  async updateOrderPatch(
    ...
    if (updates.telefono_ritiro !== undefined) {
      parts.push(
        Prisma.sql`telefono_ritiro = ${updates.telefono_ritiro ?? null}`,
      );
    }
    ...
    if (updates.tipo_pagamento !== undefined) {
      parts.push(
        Prisma.sql`tipo_pagamento = ${updates.tipo_pagamento ?? null}`,
      );
    }
    if (updates.indirizzo_consegna !== undefined) {
      parts.push(
        Prisma.sql`indirizzo_consegna = ${updates.indirizzo_consegna ?? null}`,
      );
    }
    ...
    if (updates.stato_consegna !== undefined) {
      parts.push(
        Prisma.sql`stato_consegna = ${updates.stato_consegna ?? null}`,
      );
    }
  ...
  async listRuoliPizzeriaCore(jwt: JwtOperativeUser, queryTenant?: string) {
    ...
    try {
      ...
      return (rows || []).map((u) => {
        ...
        const puoAdmin =
          rr === 'SUPERADMIN' || rr === 'OWNER' || rr === 'ADMIN';
        return {
          user_id: u.user_id,
          email: u.email ?? '',
  ...
  async righeAggregate(
    ...
    const rows = await this.prisma.$queryRaw<
      Array<{ ordine_id: string; qty: number }>
    >(
      Prisma.sql`
