...
export class PublicMenuController {
  ...
  @Get('for-domain')
  @ApiOperation({ summary: 'Menu vetrina per hostname (RPC get_public_menu_for_domain)' })
  forDomain(@Query('host') host: string) {
    return this.menu.menuForDomain(host)
  }

  @Get('categories/:tenantId')
  @ApiOperation({ summary: 'Categorie catalogo vetrina (get_public_categories_for_tenant)' })
  categories(@Param('tenantId') tenantId: string) {
    return this.menu.categoriesForTenant(tenantId)
  }

  @Post('ingredient-names')
  @ApiOperation({ summary: 'Nomi ingredienti per prodotti (get_public_menu_ingredient_names)' })
